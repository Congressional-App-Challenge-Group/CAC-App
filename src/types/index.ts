...
export type Decision = { id:string; slug:string; title:string; organization:Organization; topic:string; status:DecisionStatus; summary:string; whyItMatters:string; latestUpdate:string; nextStep:string|null; publishedAt:string; updatedAt:string; affectedRoles:UserRole[]; affectedZipCodes:string[]; affectedAreas:string[]; importantDates:ImportantDate[]; beforeText?:string; afterText?:string; isPublished:boolean; timeline:TimelineEvent[]; sources:CivicSource[] };
